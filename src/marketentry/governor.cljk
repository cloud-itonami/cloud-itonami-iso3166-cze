(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a Czechia-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Czechia-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-eu-establishment?
                                       true` (near-universal for Czechia
                                       ...
                                       Grounded in EU establishment or Czech
                                       IČO entity for many procedures
                                       (contracting authorities / MMR).
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(ICO/統一資格/NEN登録/代理人確認等)が充足していない状態での提案"}]))))
...
(defn- eu-establishment-missing-violations
  ...
  `:requires-eu-establishment?` ground truth (most Czechia public
    ...
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-eu-establishment? e))
        ...
        [{:rule :eu-establishment-missing
          :detail (str subject " はチェコ居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- ico-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-ico? true`, INDEPENDENTLY check
  `:ico-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-ico? e))
        ...
        [{:rule :ico-unverified
          :detail (str subject " はICO確認を要するが未確認 -- 提出提案は進められない")}]))))
